...
void CLIApplication::print_usage() {
    std::cout << Version::get_version_string() << "\n\n"
              << "Usage: plf_cleanup_cli [options] <base_directory>\n\n"
              << "Arguments:\n"
              ...
              << "Examples:\n"
              << "  plf_cleanup_cli ~/projects                    # Scan and delete with confirmation\n"
              << "  plf_cleanup_cli --dry-run ~/projects          # Scan only, no deletion\n"
              << "  plf_cleanup_cli --force ~/projects            # Delete without confirmation\n"
              << "  plf_cleanup_cli --folders node_modules,dist ~/projects  # Custom folder list\n"
              << "  plf_cleanup_cli --verbose ~/projects          # Show detailed progress\n\n"
              << "Exit Codes:\n"
              << "  0 - Success\n"
